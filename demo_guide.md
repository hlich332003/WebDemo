...
### Tìm hiểu về RabbitMQ (Mở rộng) — trạng thái trong dự án hiện tại (Cập nhật)

Sau khi kiểm tra codebase thực tế, tôi tổng hợp trạng thái và hướng dẫn thao tác an toàn cho RabbitMQ và Redis (mục mở rộng) như sau.

RabbitMQ — trạng thái hiện tại

- ✅ RabbitMQ đã được tích hợp vào dự án và đang được sử dụng để xử lý email bất đồng bộ (đã kiểm chứng bằng logs và code):

  - Producer:
    - `src/main/java/com/mycompany/myapp/service/MessageProducer.java` — `sendUserRegisteredEvent`, `sendOrderCreatedEvent` (gửi vào exchange `app.exchange`).
  - Config:
    - `src/main/java/com/mycompany/myapp/config/RabbitMQConfig.java` — queues, exchanges, bindings, DLQ. Tên queue hiện có: `order.email.queue`, `user.registration.queue`.
  - Consumer (email):
    - `src/main/java/com/mycompany/myapp/service/EmailConsumer.java` — lắng nghe `order.email.queue` và `user.registration.queue`, gọi `MailService` để gửi mail.
  - Chứng cứ vận hành: logs (producer gửi, consumer nhận, MailService được gọi). Nếu bạn thấy logs như `Sent user registered event for user:` và `Received user registered event for user:` tức là luồng message hoạt động.

- ⚠️ Phần xử lý nghiệp vụ bất đồng bộ cho "đặt hàng" (order-processing beyond email) — trạng thái:

  - Producer có sẵn (`MessageProducer.sendOrderCreatedEvent`) và được gọi từ `OrderService` (producer side).
  - Tuy nhiên hiện tại consumer chỉ thực thi gửi email (EmailConsumer). Các bước nghiệp vụ mở rộng (validate payment, giảm tồn kho, tạo invoice, ghi audit) chưa có consumer chuyên dụng.
  - Kết luận: Email via RabbitMQ ✅; Order-processing bất đồng bộ (full business flow) — chưa hoàn tất (partial) ❌.

- Đề xuất an toàn để mở rộng (không phá vỡ chức năng hiện tại):
  1. Tạo consumer riêng cho xử lý order (ví dụ `OrderProcessingConsumer`) lắng nghe routing key `order.created` và thực hiện các bước: validate payment (hoặc enqueue payment job), giảm tồn kho (transactional), tạo invoice và publish downstream events (nếu cần). File starter:
     - `src/main/java/com/mycompany/myapp/service/OrderProcessingConsumer.java` (mẫu)
  2. Sử dụng transaction và optimistic locking khi giảm tồn kho để tránh race condition.
  3. Kích hoạt consumer mới theo profile (ví dụ `@Profile("order-processor")`) hoặc bật/tắt qua config để không làm ảnh hưởng instance hiện đang chạy. Ví dụ trong `OrderProcessingConsumer` thêm `@Profile("order-processor")` — khi deploy bạn có thể bật profile này chỉ trên các instance chuyên xử lý background jobs.
  4. Reuse cấu hình retry / DLQ (đã có trong `RabbitMQConfig`) để chuyển message lỗi vào DLQ, tránh mất dữ liệu.

Redis — trạng thái hiện tại

- ✅ Redis được cấu hình và đang được dùng cho Spring Cache (`@Cacheable` / `@CacheEvict`) — đã bật trong `application.yml` (profile dev) và xuất hiện annotation trong nhiều file:

  - Cấu hình Redis: `src/main/resources/config/application.yml` (mục `spring.data.redis`)
  - Ví dụ sử dụng cache:
    - `src/main/java/com/mycompany/myapp/service/ProductService.java` — `@Cacheable(value = "featuredProducts")` và `ProductResource/ProductService` cho `@Cacheable("products")`.
  - Kết luận: Redis caching ✅ và hoạt động (dùng để tăng tốc truy vấn API như `GET /api/products`).

- ❌ Redis Session Store (`spring-session-data-redis`) — chưa triển khai

  - Ứng dụng hiện dùng JWT (stateless) cho authentication, không cần session server-side. Nếu bạn muốn lưu session (ví dụ cho các phiên có trạng thái server-side hoặc share session giữa nhiều instance), cần tích hợp `spring-session-data-redis`.
  - Hướng dẫn bật an toàn (không phá vỡ hiện trạng):
    1. Thêm dependency vào `pom.xml`:
       ...
       ```
    2. Bật cấu hình (ví dụ trong profile `redis-session`):
       - `application-redis-session.yml` (hoặc dùng biến môi trường) để cấu hình Redis connection (đã có `spring.data.redis`).
    3. Tạo class cấu hình:
       ```java
       @Configuration
       @EnableRedisHttpSession(maxInactiveIntervalInSeconds = 1800)
       @Profile("redis-session")
       public class RedisHttpSessionConfig {
         // nếu cần customize RedisConnectionFactory, khai báo bean ở đây
       }
       ```
    4. Chạy backend với profile `redis-session` (ví dụ `-Dspring.profiles.active=dev,redis-session`) để bật tính năng session. Nếu không bật profile này, behavior hiện tại (JWT stateless) không thay đổi.

- ❌ Redis Pub/Sub cho notifications — chưa triển khai
  - Ứng dụng có WebSocket (STOMP) nhưng nếu cần đồng bộ notifications giữa nhiều instance, bạn có thể dùng Redis Pub/Sub làm message bus giữa các instance WebSocket.
  - Hướng dẫn triển khai ngắn:
    1. Sử dụng `RedisMessageListenerContainer` và `MessageListenerAdapter` để subscribe kênh Redis.
    2. Publish message từ backend vào channel, và component WebSocket (hoặc service push) subscribe và đẩy tới client.
    3. Thực hiện theo profile (ví dụ `websocket-redis`) để bật tính năng này chỉ khi cần.

Bằng chứng file liên quan (tóm tắt)

- RabbitMQ:

  - `src/main/java/com/mycompany/myapp/config/RabbitMQConfig.java`
  - `src/main/java/com/mycompany/myapp/service/MessageProducer.java`
  - `src/main/java/com/mycompany/myapp/service/EmailConsumer.java` (đang gửi email)
  - Logs: backend console hiển thị `Sent user registered event for user:` và `Received user registered event for user:` khi test.

- Redis (cache):
  - `src/main/resources/config/application.yml` (phần `spring.data.redis` và `spring.cache.type: redis`)
  - `src/main/java/com/mycompany/myapp/service/ProductService.java` (ví dụ `@Cacheable`)

An toàn khi triển khai / bật tính năng mới

- Luôn bật tính năng mới (order-processing consumer, Redis session, Redis Pub/Sub) theo Profile hoặc config flag để không thay đổi hành vi của các instance đang chạy.
- Kiểm tra trên môi trường staging trước khi bật trên production.
- Khi thêm consumer xử lý nghiệp vụ (ví dụ giảm tồn kho), đảm bảo transactional boundaries và lock (optimistic lock) để tránh race conditions.
- Khi bật Redis session, lưu ý xung đột với JWT — bạn chỉ nên dùng session server-side nếu muốn chuyển từ JWT sang session-based auth hoặc dùng kết hợp cho các tính năng khác (ví dụ store CSRF tokens, ephemeral data).

Bước tiếp theo đề xuất (tôi có thể giúp):

1. Tạo `OrderProcessingConsumer` mẫu (tập trung xử lý nghiệp vụ: validate payment, giảm tồn kho, publish events). Tôi sẽ implement consumer mẫu và chạy thử trong profile `order-processor`.
2. Thêm hướng dẫn và ví dụ cấu hình để bật Redis Session (`spring-session-data-redis`) trong môi trường dev/staging (profile `redis-session`).
3. Triển khai Redis Pub/Sub mẫu cho notifications và tích hợp với WebSocket controller nếu bạn muốn demo realtime multi-instance.
