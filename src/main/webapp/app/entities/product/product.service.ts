...
export class ProductService {
  ...
  update(product: IProduct): Observable<EntityResponseType> {
    return this.http.put<IProduct>(
      `${this.resourceUrl}/${this.getProductIdentifier(product)}`,
      product,
      { observe: 'response' },
    );
  }
  ...
  toggleFeatured(id: number): Observable<EntityResponseType> {
    return this.http.patch<IProduct>(
      `${this.resourceUrl}/${id}/toggle-featured`,
      null,
      { observe: 'response' },
    );
  }
  ...
  compareProduct(
    o1: Pick<IProduct, 'id'> | null,
    o2: Pick<IProduct, 'id'> | null,
  ): boolean {
    return o1 && o2
      ? this.getProductIdentifier(o1) === this.getProductIdentifier(o2)
      : o1 === o2;
  }

  addProductToCollectionIfMissing<
    Type extends Pick<IProduct, 'id'> & Record<string, unknown>,
  >(
    ...
    if (products.length > 0) {
      const productCollectionIdentifiers = productCollection.map(
        (productItem) => this.getProductIdentifier(productItem),
      );
      ...
      return [...productCollection, ...productsToAdd];
    }
