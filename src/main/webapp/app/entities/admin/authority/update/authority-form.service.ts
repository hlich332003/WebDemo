 ...
 */
type PartialWithRequiredKeyOf<T extends { name: unknown }> = Partial<
  Omit<T, 'name'>
> & { name: T['name'] };
 ...
 */
type AuthorityFormGroupInput =
  | IAuthority
  | PartialWithRequiredKeyOf<NewAuthority>;
...
export class AuthorityFormService {
  createAuthorityFormGroup(
    authority: AuthorityFormGroupInput = { name: null },
  ): AuthorityFormGroup {
  ...
  resetForm(
    form: AuthorityFormGroup,
    authority: AuthorityFormGroupInput,
  ): void {
