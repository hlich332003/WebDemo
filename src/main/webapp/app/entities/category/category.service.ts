...
export class CategoryService {
  ...
  partialUpdate(category: Partial<ICategory>): Observable<EntityResponseType> {
    ...
    return this.http.patch<ICategory>(`${this.resourceUrl}/${id}`, category, { observe: 'response' });
  }
  ...
  getFeaturedCategories(): Observable<EntityArrayResponseType> {
    // Thêm phương thức này
    return this.http.get<ICategory[]>(`${this.resourceUrl}/featured`, { observe: 'response' });
  }
  ...
  addCategoryToCollectionIfMissing<Type extends Pick<ICategory, 'id'> & Record<string, unknown>>(
    ...
    if (categories.length > 0) {
      ...
      return [...categoryCollection, ...categoriesToAdd];
    }
